...
def parse_events(sock, loop_count=1000):
    ...
    for i in range(0, loop_count):
        pkt = sock.recv(1000)
	#print ''.join(i.encode('hex') for i in pkt[:])
        ...
        if event == bluez.EVT_INQUIRY_RESULT_WITH_RSSI:
		    ...
		    sender_addr=packed_bdaddr_to_string(pkt[report_pkt_offset + 3:report_pkt_offset + 9])
		    #print sender_addr
		    if sender_addr=="b8:27:eb:0c:a2:24":
                    	print "report", i
                   	...
                   	print "found our device"
                    	print ''.join(i.encode('hex') for i in pkt[report_pkt_offset +10:])

                    	report_data_length, = struct.unpack("B", pkt[report_pkt_offset + 9])
                    	print "\tadvertising packet metadata length: ", report_data_length
		    	print ''.join(i.encode('hex') for i in pkt[report_pkt_offset +10+11:])
		    	print ''.join(i.encode('hex') for i in pkt[report_pkt_offset:])
			IPsocket.sendto(''.join(i.encode('hex') for i in pkt[report_pkt_offset +10+11:]),(UDP_IP,UDP_PORT))
		    	#for index in range(0,report_data_length):
                    		#print "\tArzad data : ", (struct.unpack("B", pkt[report_pkt_offset +9+index])[0])
		    		#print ':'.join(i.encode('hex') for i in str((struct.unpack("B", pkt[report_pkt_offset +9+index])[0])))
		    		#print ':'.join(i.encode('hex') for i in pkt[report_pkt_offset +9+index])
...
#hci_connect_le(sock, "B8:27:EB:0C:A2:24")
parse_events(sock)
